	...
	class Faqs {
		...
		function add($redireciona='') {
			if(isset($_POST['acao']) && $_POST['acao'] == 'addFaq') {
				...
				$faq = filter_input(INPUT_POST, 'faq');
					try{
						...
						$sql = "INSERT INTO tbl_faq (titulo, descricao, status, id_servico, foto, faq) VALUES (?, ?, ?, ?, ?, ?)";   
						$stm = $this->pdo->prepare($sql);   
		...
		function editar($redireciona='faqs.php') {
			if(isset($_POST['acao']) && $_POST['acao'] == 'editaFaq') {
				...
				try { 
					...
					$sql = "UPDATE tbl_faq SET titulo=?, descricao=?, status=?, id_servico=?, foto=?, faq=? WHERE id=?";   
					$stm = $this->pdo->prepare($sql);   
					$stm->bindValue(1, $titulo);   
					$stm->bindValue(2, $descricao);
					$stm->bindValue(3, $status);
