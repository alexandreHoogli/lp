	...
	class Doutores {
		...
		function add($redireciona='') {
			if(isset($_POST['acao']) && $_POST['acao'] == 'addDoutor') {
				...
				if(isset($_POST['curriculo']) && !empty($_POST['curriculo'])){
                ...
                if($_FILES["foto"]["size"] > $maximo) {
					...
					try{
						...
						$sql = "INSERT INTO tbl_doutores (foto, nome, especialidade, curriculo, sexo, meta_title, meta_keywords, meta_description, graduacao, linguagem, dias_trabalho, telefone, email, url_amigavel, breve, instagram, facebook, linkedin, twitter, pertence_ao, id_categoria, cargo, ordem, id_hierarquia, foto2, embed) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";   
						$stm = $this->pdo->prepare($sql);   
		...
		function editar() {
			if(isset($_POST['acao']) && $_POST['acao'] == 'editaDoutor') {
				...
				try { 
					...
					$sql = "UPDATE tbl_doutores SET foto=?, nome=?, especialidade=?, curriculo=?, sexo=?, meta_title=?, meta_keywords=?, meta_description=?, graduacao=?, linguagem=?, dias_trabalho=?, telefone=?, email=?, url_amigavel=?, breve=?, instagram=?, facebook=?, linkedin=?, twitter=?, pertence_ao=?, id_categoria=?, cargo=?, ordem=?, id_hierarquia=?, foto2=?, embed=? WHERE id=?";   
					$stm = $this->pdo->prepare($sql);   
					$stm->bindValue(1, upload('foto', $pastaArquivos, 'N'));   
					$stm->bindValue(2, $nome);   
					$stm->bindValue(3, $especialidade);   
